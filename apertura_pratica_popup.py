# apertura_pratica_popup.py — JSON-only + ID registry + collisioni + MODIFICA PRATICA NEL POPUP
...
def mostra_popup_apertura(pratica_data: dict, id_predefinito: str, on_set_user_label, anagrafica_data: dict) -> None:
    ...
    with dialog, ui.card().classes('w-[95vw] h-[92vh] max-w-[98vw]').style('resize: both; overflow: auto;'):
        ...
        with ui.row().classes('w-full gap-4 no-wrap items-start overflow-auto'):
            ...
            # ===========================================
            # COLONNA B: MODIFICA PRATICA ESISTENTE (NUOVO)
            # ===========================================
            with ui.column().classes('flex-1 min-w-[460px]'):
                ...
                def _handle_upload(e):
                    # 1) Estrai i bytes dal payload (compat con versioni NiceGUI diverse)
                    try:
                    ...
                    # 2) Persisti nello storage utente per sopravvivere al reload
                    try:
                    ...
                    # 3) Applica subito allo stato corrente per feedback immediato
                    ...
                    # 4) Aggiorna eventuali UI collegate e chiudi il popup per mostrare la scheda pratica
                    try:
                        # Funzioni di refresh registrate in pratica.py (se presenti)
                    ...
                    try:
                        ...
                        pass
                    # Forza il re-render dell'interfaccia principale
                    try:
                ...
                def _load_from_path():
                    ...
                    # Aggiorna eventuali UI collegate e chiudi il popup per mostrare la scheda pratica
                    try:
                        # Funzioni di refresh registrate in pratica.py (se presenti)
                        pratica_data.get('refresh_pratica', lambda: None)()
                    except Exception:
