    ...
    with dialog, ui.card().classes('w-[95vw] h-[92vh] max-w-[98vw]').style('resize: both; overflow: auto;'):
        ...
        with ui.row().classes('w-full gap-4 no-wrap items-start overflow-auto'):
            ...
            with ui.column().classes('flex-1 min-w-[460px]'):
                ...
                # Import SQL: una sola istanza del widget
                try:
                ...
                def _handle_upload(e):
                    ...
                    # 2) Persisti e applica allo stato
                    try:
                    ...
                    # 3) Refresh collegati e chiudi popup (come SALVA)
                    try:
                ...
                def _load_from_path():
                    from pathlib import Path as _P
                    p = _P((path_state.get('path') or '').strip())
                    if not p.exists():
...
# === CANONICAL === inject_import_sql_carica (pulita, sincrona, chiude il dialog) ===
_import_sql_widget_injected = globals().get('_import_sql_widget_injected', False)

def inject_import_sql_carica(*, container) -> None:
    """
    Importa pratica da .sql (sincrono, nessun task in background), poi:
      - prova ad aprire SUBITO la pratica importata
      - applica i dati alla UI con _apply_db_pratica_to_state(...)
      - chiude il dialog principale (come SALVA)
    Tutte le interazioni UI avvengono dentro `with container:` per evitare errori di slot.
    """
    ...
    # DB PATH coerente con il progetto
    try:
    ...
    def _fallback_last_id() -> Optional[str]:
        try:
    ...
    def _raw_load_record(pid: str) -> Optional[Dict[str, Any]]:
        try:
            with sqlite3.connect(DB_PATH) as con:
                con.row_factory = sqlite3.Row
        ...
        if pid and callable(_load_pratica_db):
            try:
                with sqlite3.connect(DB_PATH) as con:
                    con.row_factory = sqlite3.Row
                    try:
                        record = _load_pratica_db(pid, conn=con)  # preferito
                    except TypeError:
                        record = _load_pratica_db(pid)            # fallback
            except Exception:
                record = None
        if record is None and pid:
            record = _raw_load_record(pid)
